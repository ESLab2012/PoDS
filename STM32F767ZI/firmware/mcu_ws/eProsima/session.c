...
static bool listen_message_reliably(
        uxrSession* session,
        int poll_ms);
static bool listen_message_new(
        uxrSession* session,
        int poll_ms);
static bool listen_message_reliably_new(
        uxrSession* session,
        int poll_ms);
...
static bool recv_message(
        ...
        size_t* length,
        int poll_ms);
static bool recv_message2(
        const uxrSession* session,
        int pos,
        int length,
        uint8_t** buffer,
        int poll_ms);
static bool recv_message_new(
        const uxrSession* session,
        uint8_t* buffer,
        size_t* length,
        int* priority,
        int poll_ms);
static void write_message_header(
        const uxrSession* session, 
        int priority, 
        int pos);
...
static void read_submessage_list_new(
        uxrSession* session,
        ucdrBuffer* submessages,
        uxrStreamId stream_id,
        int* priority);
static void read_submessage(
        ...
        uint16_t length,
        uint8_t flags);
static void read_submessage_new(
        uxrSession* session,
        ucdrBuffer* submessage,
        uint8_t submessage_id,
        uxrStreamId stream_id,
...
static void read_submessage_data(
        ...
        uxrStreamId stream_id,
        uint8_t format);
static void read_submessage_data_new(
        uxrSession* session,
        ucdrBuffer* submessage,
        uint16_t length,
        uxrStreamId stream_id,
...
void uxr_set_topic_callback_new(
        uxrSession* session,
        uxrOnTopicFunc_new on_topic_func_new,
        void* args_new)
{
    session->on_topic_new = on_topic_func_new;
    session->on_topic_args_new = args_new;
}
...
bool uxr_run_session_until_data_new(
        uxrSession* session,
        int timeout_ms)
{
    UXR_LOCK_SESSION(session);
    ...
    do
    {
#if(DMA_DAEMON_RECEIVE == 1)
        listen_message_reliably_new(session, remaining_time);
#elif(DMA_DAEMON_RECEIVE == 0)
        listen_message_reliably(session, remaining_time);
#endif
        if (session->on_data_flag)
        {
...
bool uxr_run_session_until_confirm_delivery_new(
        uxrSession* session,
        int timeout_ms,
        int priority)
{
    ...
    uxr_flash_output_streams_new(session, priority);
...
void uxr_flash_output_streams_new(
        uxrSession* session, int priority)
{
    UXR_HANDLE_SHARED_MEMORY();
    ...
    for (uint8_t i = 0; i < session->streams.output_best_effort_size; ++i)
    {
        ...
        {
            uxr_stamp_session_header(&session->info, id.raw, seq_num, buffer);
            send_message_new(session, buffer, length, priority, 0);
        }
    ...
    for (uint8_t i = 0; i < session->streams.output_reliable_size; ++i)
    {
        ...
        {
            uxr_stamp_session_header(&session->info, id.raw, seq_num, buffer);
            send_message_new(session, buffer, length, priority, 1);
        }
...
bool listen_message_new(
        uxrSession* session,
        int poll_ms)
{
    UXR_UNLOCK_ALL_INPUT_STREAMS(session);
    //执行此函数时，DMA Buffer中一定存在未收集的完整数据包
    uint8_t header_data[HEADER_SIZE];
    int length, hisght_length;
    int priority = -1, highest_priority = -1, highest_head;
    //未收集的数据包分为两种：未收集已处理、未收集未处理
    //处理未处理数据包的头部，将数据包的长度和优先级写入数据包头部，并作标记
    //从未收集的数据包中选择优先级最高的数据包，进行CRC校验等收集操作
    //highest_head用来记录具有最高优先级的消息的起始地址
    temp_tail = dma_head;
    temp_head = temp_tail;
    //pos用来记录该数据包的起始地址
    int pos;
    //通过recv_message_new()函数，获得数据包的长度和数据填充部分的前20个字节
    while(recv_message_new(session, header_data, &length, &priority, poll_ms)){
        //recv_message_new函数更新temp_tail，使temp_tail指向下一个数据包的起始位置
        if(priority == 0){
            //优先级等于0，说明该数据包未被处理过
            if(length < 50){
                //如果该数据包的长度小于50，说明该数据包不是正常消息，不具有优先级，直接进行处理即可
                //记录该数据包的起始地址
                pos = temp_head;
                uint8_t* data;
                //将具有最高优先级的数据包的数据填充部分复制出来
                recv_message2(session, pos, length, &data, poll_ms);
                ucdrBuffer ub;
                ucdr_init_buffer(&ub, data, length);
                read_message(session, &ub);
                temp_head = temp_tail;
                continue;
            }
            //说明该数据包未经过处理，获得该数据包的优先级
            ucdrBuffer temp_ub;
            uxrSession temp_session = (* session);
            ucdr_init_buffer(&temp_ub, header_data, HEADER_SIZE);
            read_message_new(&temp_session, &temp_ub, &priority);
            //记录该数据包的起始地址
            pos = temp_head;
            //标记该数据包，并将数据包的长度和优先级写入数据包头部
            write_message_header(session, priority, pos);
        }else if(priority != -1){
            //说明该数据包经过处理，但是未被收集
            //记录该数据包的起始地址
            pos = temp_head;
        ...
        if(priority > highest_priority){
            ...
            highest_head = pos;
            hisght_length = length;
            priority = 0;
        }
        //更新temp_head，使其指向下一个数据包的起始地址，开始操作下一个数据包
        temp_head = temp_tail;
    }
    //对具有最高优先级的数据包进行CRC校验和收集操作
    if(highest_priority != -1){
        uint8_t* data;
        //将具有最高优先级的数据包的数据填充部分复制出来
        recv_message2(session, highest_head, hisght_length, &data, poll_ms);
        ucdrBuffer ub;
        ucdr_init_buffer(&ub, data, hisght_length);
        read_message(session, &ub);
    }
    UXR_UNLOCK_ALL_INPUT_STREAMS(session);
...
bool listen_message_reliably_new(
        uxrSession* session,
        int poll_ms)
{
    bool received = false;
    ...
    do
    {
        ...
        int poll_chosen = (poll_to_next_heartbeat < poll) ? poll_to_next_heartbeat : poll;
        received = listen_message_new(session, poll_chosen);
        if (!received)
        {
            poll -= poll_chosen;
        }
...
inline bool send_message_new(
        ...
        int8_t qos)
{
    bool sent = session->comm->send_msg_new(session->comm->instance, buffer, length, priority, qos);
...
inline bool recv_message2(
        ...
        int poll_ms)
{
    bool received = session->comm->recv_msg2(session->comm->instance, buffer, pos, length, poll_ms);
...
inline bool recv_message_new(
        ...
        int poll_ms)
{
    bool received = session->comm->recv_msg_new(session->comm->instance, buffer, length, priority, poll_ms);
...
inline void write_message_header(
    const uxrSession* session, 
    int priority, 
    int pos)
{
    session->comm->write_msg_header(session->comm->instance, pos, priority);
}
        ...
        int* priority)
{
    ...
    {
        uxrStreamId id = uxr_stream_id_from_raw(stream_id_raw, UXR_INPUT_STREAM);
        read_stream_new(session, ub, id, seq_num, priority);
    }
}
        ...
        int* priority)
{
    switch (stream_id.type)
    {
        ...
        case UXR_BEST_EFFORT_STREAM:
        {
            ...
            if (stream)
            {
                read_submessage_list_new(session, ub, stream_id, priority);
            }
            break;
        }
        case UXR_RELIABLE_STREAM:
        ...
        int* priority)
{
    ...
    while (uxr_read_submessage_header(submessages, &id, &length, &flags))
    {
        read_submessage_new(session, submessages, id, stream_id, length, flags, priority);
    }
}
        ...
        int* priority)
{
    switch (submessage_id)
    {
        ...
        case SUBMESSAGE_ID_DATA:
            read_submessage_data_new(session, submessage, length, stream_id, flags & FORMAT_MASK, priority);
            break;
...
extern void read_submessage_format(
        ...
        uxrObjectId object_id,
        uint16_t request_id);
extern void read_submessage_format_new(
        uxrSession* session,
        ucdrBuffer* data,
        uint16_t length,
        uint8_t format,
        ...
        int* priority)
{
    ...
    process_status(session, object_id, request_id, UXR_STATUS_OK);
    read_submessage_format_new(session, submessage, length, format, stream_id, object_id, request_id, priority);
}

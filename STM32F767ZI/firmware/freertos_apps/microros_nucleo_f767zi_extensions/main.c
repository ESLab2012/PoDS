...
#if(DMA_DAEMON_RECEIVE == 0 && DMA_DAEMON_SEND == 0)
...
#else
  availableNetwork = true; 
  rmw_uros_set_custom_transport_new( 
    true, 
    (void *) &huart3, 
    freertos_serial_open, 
    freertos_serial_close, 
    freertos_serial_write,
    freertos_serial_write_new, 
    freertos_serial_read,
    freertos_serial_read_new,
    write_to_dma_buffer); 
#endif
